...
| M2.07 | Independent reviewer `review_interfaces` | Early CLI output reduced new relationship evidence to counts; inspected assessments and full non-CLI reference coverage were unfinished | Integration builder is completing identity-bearing prose, qualified assessments, and reference-resolution tests before handoff |
| M2.07 | Independent reviewer and coordinator interactive smoke | Missing pin changes and assessment access; incidental PV facts hid mate/subject assessments; removed undefended facts implied gained defence; one failure sentence overstated reply coverage | Presentation fixes use shared helpers and catalog wording; each sentence must match the typed producer predicate, with exact counterexamples |
| M2.07 | Coordinator | Consecutive presentation and reference-test gaps repeated earlier acceptance-evidence weaknesses | Require shared presentation assembly and real produced evidence with nonempty, resolvable targets; no broader chess refactor is justified by these interface defects |
| M2.08 | Integration test builder `integration_gate` | Unicode multi-game workflow, exact source hashes, malformed settings preservation, promotion/back, comparison/preview, flips, probes, engine failure/retry, invalid imports, and capacity boundary: 3 integration tests passed | Final ordinary/engine suite and independent review remain required |
| M2.08 | Integration test builder `integration_gate` | Fifty probe/analyze replacements plus ten navigation submissions reached current request 60; maximum one active and one pending request; candidates/nodes stayed bounded; active/pending/worker cleared after settlement and close | Automated replacement gate passed; real-engine process settlement is covered separately |
| M2.08 | Fresh independent reviewer `final_arch_review` | Full M2 diff reviewed for chess claims, boundaries, settings, history, references and lifecycle; 169 focused passed, 2 skipped; 271 ordinary passed, 10 skipped; 590 generated legal transitions retained valid deltas and identity | No material architecture/chess defect found; interface corrections and final engine gate remain separate requirements |
